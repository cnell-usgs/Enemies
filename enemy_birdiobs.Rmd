...
```{r,datasummary,warning=F,include=F,echo=F}
...
```{r, fguilds,fig.height=4,message=F,echo=F}
...
```
<br>  
```{r,diagnostics,fig.height=4,echo=F}
...
```{r,plotplots,fig.height=3.5,message=F,echo=F}
...
```{r,abunbydivd, results="asis",echo=F}
...
```{r,divplots,fig.height=3.5,echo=F}
...
```{r,abunbydivfg, results="asis",echo=F}    
rtr<-lm(total_birds_visit~DIVERSITY*feeding.guild+PLOT,data=plot.guild2)
pander(summary(aov(rtr)),round=c(0,2,2,2,3)) 
```
 <style type="text/css">
.table {
    width: 50%;
...
birds.div.vars<-birds.plot.mean.vars%>%
  group_by(DIVERSITY)%>%
  summarize(total_abun = sum(total_abun),mean_abun_div = mean(mean_abun), se_abun_div = std(mean_abun),mean_div_div = mean(mean_div), se_div_div = std(mean_div), mean_rich_div = mean(mean_rich), se_rich_div = std(mean_rich))
...
```{r,allbird,fig.height=4,message=F,comment=F,echo=F}
...
**Richness estimators:**    
```{r,richnest,results="asis",echo=F}
...
<br>  
```{r,richhist,fig.height=4,echo=F}
...
div.rich<-ggplot(data=birds.div.vars,aes(x=DIVERSITY,y=mean_rich_div,color=DIVERSITY))+##richness plot by diversity
  geom_point(size=10,shape=19)+
...
ANOVA testing for main effect of tree DIVERSITY controlling for PLOT    
```{r,richbydivd, results="asis",echo=F}
richbydiv<-aov(richness~DIVERSITY+PLOT,data=birds)
pander(summary(richbydiv))  
```
<style type="text/css">
.table {
    width: 50%;
...
```{r,richguild,fig.height=3.5,eval=F,echo=F}


guildrich<-ggplot(div.guild,aes(y=total_birds,x=reorder(feeding.guild,total_birds),fill=DIVERSITY))+
  geom_bar(stat="identity")+
  scale_fill_manual("Plot Diversity",labels=c("Monoculture","Polyculture"),values=c("darkslateblue","#1B9E77"))+
  theme_minimal()+theme(legend.position="top")+labs(x="Feeding Guild",y="Total Birds Observed")

richguild.plot<-ggplot(plot.guild3,aes(x=reorder(feeding.guild,mean_birds_div),y=mean_birds_div,fill=DIVERSITY))+
  geom_bar(stat="identity",position="dodge")+theme_minimal()+scale_fill_manual(values=c("darkslateblue","#1B9E77"))+
  geom_errorbar(aes(ymin=mean_birds_div-se_birds_div,ymax=mean_birds_div+se_birds_div),width=.2,position=position_dodge(.9))+theme(legend.position="none")+
  labs(x="Feeding Guild",y="Birds/Visit")

plot_grid(guildrich,richguild.plot)
```  
