...
hist.div<-ggplot(bird.visit,aes(x=total_abun,fill=DIVERSITY,color=DIVERSITY),alpha=.6)+geom_density(alpha=.7)+
  ...
  scale_fill_manual("Plot Diversity",labels=c("Monoculture","Polyculture"),values=c("darkslateblue","#1B9E77"))+scale_color_manual("Plot Diversity",labels=c("Monoculture","Polyculture"),values=c("darkslateblue","#1B9E77"))+
...
```{r,plotplots,fig.height=4,message=F}
...
guilddiv<-ggplot(div.guild,aes(y=total_birds,x=reorder(feeding.guild,total_birds),fill=DIVERSITY))+
  ...
  theme_minimal()+theme(legend.position="none")+labs(x="Feeding Guild",y="Total Birds Observed")
...
typeplot<-ggplot(compareins,aes(x=mean_birds_visit.x,y=mean_birds_visit.y,color=DIVERSITY.x))+
  geom_point(size=2)+
  geom_errorbar(aes(ymin=mean_birds_visit.y-se_birds_visit.y,ymax=mean_birds_visit.y+se_birds_visit.y),color="black")+
  geom_errorbarh(aes(xmin=mean_birds_visit.x-se_birds_visit.x,xmax=mean_birds_visit.x+se_birds_visit.x),color="black")+
  theme_minimal()+
